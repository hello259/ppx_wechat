...
Page({
    ...
    goodsSubmit: function () {
        ...
        if (that.data.selected_arr.length == that.data.goodsClassify.length) {
            ...
            // console.log(that.data.buyNumber)
    ...
    RTS: function (goods_id) {
        ...
        app.fetch(app.globalData.url + "/mobile/index.php?act=goods&op=goods_detail&t=json", obj, "post", 'applicaion/json').then(res => {
            ...
            console.log(_that.data.isStart)
        })
    },
    // 定时器
    formatTime(time) {
    ...
    handlerSubmit: function () {
        ...
        if (that.data.goodsTAG == 1 && that.data.activeState == '马上抢') {
            /* 判断是否选择完全部的商品规格 */
            if (that.data.selected_arr.length == that.data.goodsClassify.length) {
        ...
        } else if (that.data.goodsTAG == 2 && that.data.activeState == '尚未开始') {
            wx.showToast({
                title: '即将开始喔~',
                icon: 'none',
                duration: 2000
            })
        } else if (that.data.activeState == '活动已结束') {
            wx.showToast({
                title: '已经结束啦~',
                icon: 'none',
                duration: 2000
